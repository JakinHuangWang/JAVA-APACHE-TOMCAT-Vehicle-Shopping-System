	...
	public void handleConnection(Socket sock) {
		...
		try {
			do {
				...
				int request = Integer.parseInt(fromClient.toString());
				if (DEBUG)
		...
		try {
			switch (request) {
				case 1: //Client request to build Automobile
					...
					fromClient = in.readObject();
					toClient = protocol.processRequest(fromClient, 1);
					sendOutput(toClient);
					if (DEBUG)
						System.out.println("Adding new Automobile to database ... ");
					break;

				case 2: //Client request to configure Automobiles
					if (DEBUG)
						System.out.println("Start Configuring an Automobile ");
					toClient = protocol.processRequest(fromClient, 2);
					sendOutput(toClient);
					if (DEBUG)
